...
//    }
    ...
    public final int[] scanFieldIntArray(long fieldHashCode) {
        ...
        offset = offset2;
        chLocal = chLocal2;
        int[] array = new int[computeIntArraySize(offset, chLocal)];
        offset = populateIntArray(offset, array);
        // chLocal = charAt(bp + (offset++));
        charIndex = bp + (offset++);
        chLocal = charIndex >= this.len ? //
                EOI //
    ...
    protected int populateIntArray(int offset, int[] array) {
        int charIndex;
        char chLocal;
        int arrayIndex = 0;
        // chLocal = charAt(bp + (offset++));
    ...
    protected int computeIntArraySize(int offset, char chLocal) {
        int charIndex;
        int arraySize = 0;
...
//            this.next();
    ...
    public final float[] scanFieldFloatArray(long fieldHashCode) {
        ...
        float[] array = new float[computeArraySize(offset)];
        offset = populateFloatArray(offset, array);
    ...
    protected int populateFloatArray(int offset, float[] array) {
        int charIndex;
        char chLocal;
        int arrayIndex = 0;
        // chLocal = charAt(bp + (offset++));
        ...
        for (; ; ) {
            ...
            array[arrayIndex++] = computeFloatValue(start, negative, intVal, power, exp, count);
        ...
        for (;;) {
            if (chLocal == '[') {
                ...
                if (arrayarrayIndex >= arrayarray.length) {
                    ...
                    System.arraycopy(array, 0, tmp, 0, arrayarrayIndex);
                    arrayarray = tmp;
                }
    ...
    private int computePower(int offset, char chLocal) {
        int charIndex;
        int power = 1;
        boolean small = (chLocal == '.');
        if (small) {
